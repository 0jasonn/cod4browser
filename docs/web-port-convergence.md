...
GfxLightDef, MenuList, and Menu publication, while retaining
...
The production web target contains 84 C/C++ translation units: 52 outside
...
| Database initialization | `MODIFIED KISAK` / partial | The web target compiles real `db_registry.cpp` ownership for `DB_BuildOSPath`, `DB_TryLoadXFileInternal`, `DB_TryLoadXFile`, `DB_Thread`, `DB_LoadXZone`, `DB_LoadZone_f`, `DB_InitThread`, and `DB_LoadXAssets`. It then executes through XFile inflate, PMem blocks, streams, generated list/string/asset dispatch and every family encountered by the first retail zone, including XModel, XAnimParts, WeaponDef, and StringTable. Narrow synchronous file and Sys context adapters own the platform boundary. Retail `code_post_gfx.ff` now completes all 1,639 ordered assets and 1,698 real DB publications with no unsupported-family stop. |
...
| Asset registry | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | All generated families reached in `code_post_gfx.ff`, including XModel, XAnimParts, WeaponDef, and StringTable, consume the canonical 32-bit asset-entry pool, per-type pools, free chain, normalized hash table, and zone ownership through `DB_AddXAsset`/`DB_LinkXAssetEntry`. The owned run publishes 1,698 records through entry 1,713, with free entries 32,752 -> 31,054. Gate 2 remains a frozen oracle and is not called by the generated path. |
...
use the canonical DB path. The owned retail startup zone completes all 1,639
assets and 1,698 publications, ending naturally at RawFile `code_post_gfx`
with block offsets `[0,0,0,0,407412,0,0,4224,480]`. Per the architecture
review stop, the next normally requested retail zone has not yet been entered.
