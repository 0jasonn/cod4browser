...
`G_InitGame`, `G_LoadLevel`, `SV_InitGameVM`, and `CG_Init`. Gate 2 remains a
separate frozen render oracle; no game-driven frame has rendered yet.
...
| qcommon startup | `MODIFIED KISAK` / reached lifecycle plus `TEMPORARY WEB SUBSTITUTE` oracle | Real `common.cpp` executes through `PMem_Init`, `DB_SetInitializing(true)`, `$init`, `Com_InitXAssets`, and `DB_InitThread`. The Worker continuation executes canonical `FS_InitFilesystem` at its native pre-Hunk point, initializes native Hunk/profiling/script/XAnim/DObj/server owners, and runs full canonical `CL_Init` before the startup-zone request. Native engine operations remain synchronous-looking without Asyncify or Promises. |
...
| Renderer frontend | `MODIFIED KISAK` / partial platform closure | The production Wasm target links the real client/cgame/effects/ragdoll/physics closure against a narrow renderer-frontend platform implementation and now compiles canonical `r_dvars.cpp` plus the portable sun-dvar registration from `r_sky.cpp`. The bounded canonical `GfxWorld` adapter remains the frozen rendering oracle and retains no second world model. `R_RenderScene` is still a no-op: the next slice must translate the actual cgame view through portable draw commands at this frontend/backend seam; WebGL handles remain backend-private. |
...
| cgame and `CG_Init` | `SHARED KISAK` / reached initialization | The smallest linked cgame closure executes through real `CL_InitCGame -> CG_Init` and completes for the local SP client. Canonical renderer dvar and sun-dvar registration is compiled; unavailable native D3D/window behavior remains feature-gated at the browser renderer boundary. `R_RenderScene` is not yet connected to portable WebGL2 draw submission, so this is initialization parity, not a rendered game frame. |
| game and `SV_Init` | `MODIFIED KISAK` / reached game-program closure | Shared owners preserve `SV_Init`, `SV_Map_f`, map-command registration, `SV_SpawnServer`, and `SV_LoadLevelAssets`. The retail path continues through real `CM_LoadMap`, `Com_LoadWorld`, save initialization, `SV_InitGameProgs`, `SV_InitGameVM`, `G_InitGame`, five canonical settle frames, direct-connect/client-enter-world, `G_LoadLevel`, and its first server game frame. Entity, level, server, and script state remain canonical Kisak owners. |
...
rendered through the frozen Gate 2 WebGL2 oracle. The runtime pivot compiles
and executes `CM_LoadMap`, `Com_LoadWorld`, the save system, script VM, XAnim,
DObj, client, cgame, game, effects, ragdoll, physics, and sound closures. The
retail browser run reaches `G_InitGame`, completes `G_LoadLevel`,
`SV_InitGameVM`, `SV_InitGameProgs`, `CL_InitCGame`, and `CG_Init` without a
...
6. Actual cgame view/camera submission through the renderer frontend and WebGL2.
7. Browser input and audio adapters needed for an offline playable slice.
...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High: qcommon, the full Killhouse generated-family order, filesystem, collision, script VM, xanim/DObj, server/game, client/cgame, effects, ragdoll, physics, sound, and canonical renderer-dvar owners now compile and execute through initialization. The remaining gap is no longer broad runtime ownership: it is the canonical renderer frontend/draw closure plus later feature-specific input, audio, cinematic, and gameplay dependencies. |
