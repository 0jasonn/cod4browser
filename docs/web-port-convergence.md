...
GfxImage, water, LocalizeEntry, SoundCurve, sound aliases, LoadedSound, Font,
FxEffectDef, FxImpactTable, GfxLightDef, MenuList, and Menu publication, while retaining
...
The production web target contains 77 C/C++ translation units: 45 outside
...
| Asset registry | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | Generated RawFile, PhysPreset, MaterialTechniqueSet, Material, GfxImage, LocalizeEntry, SndCurve, sound alias, LoadedSound, Font, FxEffectDef, FxImpactTable, GfxLightDef, MenuList, and Menu paths consume the canonical 32-bit asset-entry pool, per-type pools, free chain, normalized hash table, and zone ownership through `DB_AddXAsset`/`DB_LinkXAssetEntry`. Owned retail publishes 1,555 records through entry 1,570, with free entries 32,752 -> 31,197. Gate 2 remains a frozen oracle and is not called by the generated path. |
...
eight-byte body/XString closure; the current retail traversal now totals 1,248
...
records those canonical closures, the lightweight UI ABI extraction, and the
new WeaponDef boundary. The current
...
sound alias, LoadedSound, Font, FxEffectDef, FxImpactTable, GfxLightDef,
MenuList, and Menu publication now use shared Kisak code. The owned retail
startup zone publishes 1,555 records and stops at asset 1506, type 23
`ASSET_TYPE_WEAPON`; the next call is `Load_WeaponDefPtr`. That 2,168-byte
body depends on the still-uncompiled canonical XModel generated closure and is
the next dependency-heavy architectural slice.
