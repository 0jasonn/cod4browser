...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High but falling: qcommon, the full Killhouse generated-family order, filesystem, collision, script VM, xanim/DObj, server/game, client/cgame, effects, ragdoll, physics, sound, and canonical renderer-dvar owners now compile and execute through gameplay. Static XModels and ordinary plus first-person dynamic DObjs are submitted through the canonical frontend; remaining renderer gaps are broader material/entity families, skinned FX/marks/decals, campaign variance, and deliberately deferred shader/postprocessing features, not a parallel browser world or camera owner. |
