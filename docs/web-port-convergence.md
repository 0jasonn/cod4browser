...
lifecycle and generated RawFile, PhysPreset, XModel, XAnimParts, WeaponDef,
...
Kisak DB now publishes Killhouse `GfxWorld` asset 772 and its DB-owned bounded
surface is drawn through WebGL2; Gate 2 remains a separate frozen oracle.
...
| Command system | `MODIFIED KISAK` / partial | Production Wasm and strict trace tests compile canonical `src/qcommon/cmd.cpp`; `cmd_core.cpp` is retired from all build lists. Native registration/lookup, tokenization, argument access, buffering, `wait`, startup commands, removal lifetime, and the shared SP map-command dispatch slice are compared under Win32 x86 and Wasm. Production now accepts runtime `map` input through those tables and reaches the normal DB boundary. Script/client forwarding, developer tooling, autocomplete, and filesystem/fastfile `exec` remain gated with their owning subsystems. |
...
| qcommon startup | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | Real `common.cpp` executes through `PMem_Init`, `DB_SetInitializing(true)`, `$init`, `Com_InitXAssets`, and `DB_InitThread`. The bounded Worker lifecycle initializes shared script/XAnim/DObj/server owners, uses canonical `CL_InitRef` configuration, and submits the renderer prerequisite request. Full `FS_InitFilesystem` and `CL_Init` are intentionally not entered until the Worker mount owns canonical search paths and archives. Native `$init` DB waiting remains synchronous-looking without Asyncify or Promises in engine code. |
...
| Database initialization | `MODIFIED KISAK` / partial | The web target compiles real `db_registry.cpp` ownership for `DB_BuildOSPath`, `DB_TryLoadXFileInternal`, `DB_TryLoadXFile`, `DB_Thread`, `DB_LoadXZone`, `DB_LoadZone_f`, `DB_InitThread`, and `DB_LoadXAssets`. Shared client configuration and renderer request construction submit `code_post_gfx`, `ui`, and `common` in native order; all 8,176 startup assets complete. A real runtime `map` command then submits `killhouse`, interns 892 script strings, publishes through canonical GfxWorld asset 772, and stops fail-closed at the next ordered GameWorldSp asset 773. Narrow synchronous file and Sys context adapters own the platform boundary. |
...
| `GfxImage` | `MODIFIED KISAK` / partial; native backend `NATIVE ONLY` | The reusable generated family now loads 36-byte image bodies, XString names, texture load definitions/payloads, aliases, insertion cells, and canonical DB publication. The complete prerequisite chain records 321 GfxImage publications. The renderer-disabled `Load_Texture` seam leaves GPU handles null; IWI decoding, WebGL resource creation, and context recovery remain backend infrastructure. |
...
| Renderer frontend | `MODIFIED KISAK` / partial platform closure | The production Wasm target now links the real client/cgame/effects/ragdoll/physics source closure against a narrow renderer-frontend platform implementation. The bounded canonical `GfxWorld` adapter remains the rendering proof and retains no second world model. Fuller portable draw-command behavior must stay behind this same frontend; WebGL handles remain backend-private. |
...
| Collision and `CM_LoadMap` | `SHARED KISAK` / compiled runtime owner | Production compiles actual `cm_load.cpp`; the DB ClipMap singleton pool is `&cm`, and successful map DB completion continues through real `CM_LoadMap`. It initializes canonical collision thread data and publishes checksum/in-use state. An exact MSVC x86/Wasm differential invokes the owner and matches all normalized results. The retail path currently stops at GameWorldSp asset 773 before ClipMap publication, so the fail-closed server continuation and browser test prevent an early call. |
| xanim and DObj runtime | `SHARED KISAK` / initialization owner compiled | `xanim_init.cpp` and `dobj_init.cpp` own the actual 4,096-entry `XAnimInfo` free ring, `end` notetrack ScriptString, and duplicate-parts ScriptString initialization. Lightweight runtime headers expose the same canonical structs without importing D3D. Production executes them in native `Com_Init` order; exact x86/Wasm evidence covers the ring and handles. Playback and skeletal evaluation remain in the larger canonical closure. |
| Script VM and `Scr_Init` | `SHARED KISAK` / initialization owner compiled | Actual variable-range setup, `AllocValue`, `Scr_Init`, `Scr_VM_Init`, and `Scr_Settings` now run in production. Canonical VM/compiler/anim public state layouts were moved to renderer-free headers, not replaced. Exact x86/Wasm evidence covers the parent/child free lists, temp value 1, VM stack bounds/type, and settings. Full script compilation/execution remains part of `G_InitGame`. |
| Client and `CL_Init` | `MODIFIED KISAK` / compiled lifecycle owner | `cl_initialization.cpp` owns the canonical `CL_Init` body and client globals, and a Win32 x86/Wasm differential runs its normalized contract identically. Production currently calls only `CL_InitRef` at the renderer fastfile boundary; entering full `CL_Init` first requires canonical filesystem search-path/archive integration. No browser-owned client state substitutes for it. |
| cgame and `CG_Init` | `SHARED KISAK` / compiled, not entered | The cgame source closure links in production through explicit renderer/input/platform seams. It is not executed in the owned browser run because the map DB has not yet published ClipMap and the local client lifecycle has not crossed full `CL_Init`. |
| game and `SV_Init` | `MODIFIED KISAK` / production map slice | Shared owners preserve `SV_Init`, `SV_Map_f`, map-command registration, `SV_SpawnServer`, and `SV_LoadLevelAssets`. After successful map DB completion, production constructs the canonical BSP name and calls real `CM_LoadMap`, `Com_LoadWorld`, save initialization, and the linked game/server closure. The owned retail run still stops at GameWorldSp before ClipMap, so none of these post-DB calls run prematurely and no partial game state is installed. |
...
canonical GfxWorld asset 772, recording 2,371 publications before stopping at
the next ordered GameWorldSp asset 773. Gate 2 remains an independent frozen
oracle used only to compare the published DB world and bounded render result.

Generated-loader convergence has reached canonical `ComWorld` and `GfxWorld`,
and the real DB-owned bounded world has rendered through WebGL2. The runtime
pivot compiles `CM_LoadMap`, `Com_LoadWorld`, the save system, script VM,
XAnim, DObj, client, cgame, game, effects, ragdoll, physics, and sound source
closures. Exact native/Wasm tests enter the bounded runtime owners; the retail
browser run correctly cannot enter post-DB collision while ClipMap remains
unpublished. Replacing the bounded startup entry with full canonical client
startup now requires an explicit Worker-filesystem architecture decision:
`FS_InitFilesystem` must consume the existing mount for search paths,
directory enumeration, and IWD/minizip access without a second browser asset
registry.

1. `Com_Init` and the real qcommon lifecycle.
2. `DB_LoadXZone` and canonical asset ownership.
3. canonical `FS_InitFilesystem` over the Worker mount.
4. `CM_LoadMap` and collision after ClipMap publication.
5. full `CL_Init`, `CG_Init`, and local game/server execution.
...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High: qcommon plus canonical DB framing, incremental inflate, PMem blocks, stream globals, generated list/string/asset dispatch, RawFile, PhysPreset, TechniqueSet, Material, GfxImage, water, LocalizeEntry, SoundCurve, sound aliases, LoadedSound, Font, FX, Impact, Light, MenuList, Menu, and their real DB publications now execute. Remaining generated families, client, cgame, game, xanim runtime, collision, and script VM remain outside the web target. |
