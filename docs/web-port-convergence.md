...
context. It does not introduce a browser impact system or a parallel
world/model representation. Animated DObj mark attachment remains explicit
future work.

Focused Wasm-native coverage exercises world receiver clipping and material
rejection, a pose-local moving-brush mark, and a transformed static-XModel mark
with its canonical EffectsCore context. Existing BSP impact marks remain
visually intact in the Release Chrome runtime.
...
impacts, ambient sounds, and UI transitions. Streaming and animated-DObj marks
are still deferred; the playable offline firing slice no longer depends on a
