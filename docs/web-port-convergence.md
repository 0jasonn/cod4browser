...
| Browser launcher and legal asset selection | `WEB PLATFORM IMPLEMENTATION` | DOM file picker, minimum retail-profile validation, complete selected-root manifest, and restore UI are genuine browser responsibilities. Production reports persistence grant, usage, quota, capacity pressure, and an explicit persistence retry without implying that browser-local files are guaranteed. The host persists generic logical paths; it does not select runtime search paths or packs. |
| OPFS/IndexedDB asset persistence | `WEB PLATFORM IMPLEMENTATION` | Keep storage asynchronous in the host and hidden behind the filesystem boundary. Validated installation files remain immutable; the separate bounded browser home tree restores from and persists to OPFS. An explicit host lifecycle retains both filesystem leases through retryable failures and unknown timeouts, terminates an uncertain Worker before release, and rejects stale-generation replies. |
| Wasm filesystem bridge | `WEB PLATFORM IMPLEMENTATION` / reached read/write closure | The engine Worker mounts every validated logical-root entry through read-only synchronous OPFS access handles and exposes stat/type, direct-child enumeration, open, size, seek, read, and close primitives. A separate synchronous-looking writable overlay owns directory creation, truncate/append, bounded writes, remove, and rename for canonical `fs_homepath` calls, snapshots closed files to OPFS asynchronously, and restores them before engine mount. Mounted profiles support non-unmounting checkpoints; concurrent dirtiness coalesces into at most one non-overlapping follow-up and never releases writer ownership. Import IDs and browser handles remain platform-only. Canonical `FS_InitFilesystem`, `FS_CreatePath`, and the ordinary save/config APIs consume this boundary without Asyncify. |
| Browser lifecycle, logging, and timing | `WEB PLATFORM IMPLEMENTATION` | Event-loop and page-lifecycle handling remain in `src/web`. Visibility-hidden profiles checkpoint without unmounting, surface failure, and retry on later dirtiness; `pagehide` remains best-effort final cleanup. `common.cpp` now owns engine print formatting while `Sys_Print`/`Sys_Error` remain narrow platform sinks. |
...
| Renderer frontend | `MODIFIED KISAK` / textured, lightmapped, shadowed, model-lit gameplay and canonical marks reached | The production Wasm target links the real client/cgame/effects/ragdoll/physics closure against a narrow renderer-frontend platform implementation. `R_RenderScene` validates canonical `refdef_s`, constructs Kisak view/projection matrices, and traverses the canonical DPVS lit, decal, and emissive ranges in native stage order rather than treating `surfaceCountNoDecal` as a contiguous endpoint. It emits 581 material-aware batches with canonical `Material*`, technique identity, base `GfxImage*`, state bits, sampler state, lightmap index, base UV, and lightmap UV. `R_LoadWorld` rebuilds the native runtime `surfaceMaterials` table and `surfaceCastsSunShadow` bitset from each canonical surface flag and material draw key; sun-enabled world draws then prefer canonical `TECHNIQUE_LIT_SUN_SHADOW` and consume that membership. Frontend-owned `R_UnloadWorld` clears world readiness, pointers, cached submissions, material references, and map flags before delegating backend map-resource release; lifecycle assertions reject a second publication over retained state and prove A -> unload -> B -> unload -> full shutdown. Static instances sample the canonical `GfxWorld::lightGrid` at their native bounds centers or retain encoded ground lighting. Ordinary and first-person `R_AddDObjToScene` submissions retain the caller's lighting origin, canonical pose evaluation, cpose/view-origin LOD selection delegated to `XModelGetLodForDist`, and rigid/weighted position plus normal skinning. EffectsCore code-mesh, XModel, particle-cloud, and persistent mark submissions remain appended in canonical order and do not enter the model-lighting branch. The mark-fragment seam clips canonical `GfxWorld` world-brush, moving-brush, static-XModel, and rigid animated-DObj receiver triangles against the six native mark planes and retains Material/lightmap/model-context identity; particle marks remain a later family. Unsupported/deformed standalone FX model surfaces, invalid/over-capacity clouds, broader material families, and deferred shader/postprocess features remain compatibility gaps. The minimum 2D callback set retains canonical Material/Font identities and publishes at `R_EndFrame`, including UI-only fullscreen and script-popup frames. |
| WebGL2 backend and context recovery | `WEB PLATFORM IMPLEMENTATION` | Permanent platform boundary. Map unload telemetry remains diagnostic-only: it measures decoded recovery bytes before/after the existing backend release seam, orders `worldUnloadBegin` → `worldUnloadEnd` → `newWorldPublished`, and records a monotonic context generation without adding a browser-owned world lifecycle. Synthetic coverage proves retained map bytes fall while the context generation stays fixed; retail measurement is emitted only by the local validation tier. The backend converts D3D9 NDC depth `[0,1]` to WebGL `[-1,1]`, retains 32-bit indices, uploads canonical external IWI and DB load-definition pixels (including encountered BGR8, A8L8, L8, and X8R8G8B8 images), draws the canonical world sky cubemap from the current view axes/FOV, and mirrors the encountered Killhouse secondary-directional lightmap decode. Dominant `lm_*s0_sm3` world passes retain semantic-8 specular maps, `envMapParms`, and each surface's canonical reflection probe. Cubemap recovery retains the complete authored smallest-to-largest IWI or face-major DB mip chain, and the backend uploads every level with mipmapped filtering so explicit specular-alpha probe LODs no longer collapse to the sharpest face. The GLSL boundary reproduces the recovered D3D9 world arithmetic and the encountered `lp_*s0_sm3` XModel arithmetic: world slope normals or model DXT5nm tangent-space normals, view reflection, specular-alpha probe LOD, RGBM multiplication, authored Fresnel, and model reflection added after `base*vertex*modelLighting*2`. Static model batches split by canonical reflection-probe identity; DObjs choose from the published BSP cell probe list. Live Killhouse retains 545 SM3-specular world batches out of 762 with all 320 world images supported. Sun shadows own native-scale 1024-square near/far depth targets: 0.25-world-unit near sampling and the default 4×/1.0-world-unit far partition, each with view-centred texel snapping and world-bound depth extents. Alpha-tested canonical world/static-XModel/ordinary-DObj/DynEntity/moving-brush casters feed both partitions; receivers choose near coverage first and fall through to the far map with four-comparison PCF. Rebuilding the runtime world-caster bitset before submission prevents direct sun from leaking through authored roof/wall casters. Encountered `lm_sm_sun_*` receivers add the native `N dot sun * sunDiffuse` term; exact per-partition static/entity DPVS culling remains an explicit gap. Encountered `lm_spot_*` world batches preserve their per-surface light identity, both lightmaps, the authored RGB attenuation-atlas row, cone, radius, color, and normal basis, and reproduce the native baked-plus-spot diffuse arithmetic. Up to four view-ranked primary spots own 512-square depth maps and four-comparison receiver filtering. The shadowed path follows retail `lm_spot_sm`: after selection fade-in, its dynamic comparison replaces the authored primary-lightmap visibility instead of multiplying both masks and producing a false hard-edged light patch. Omni variants remain a gap. The encountered `water_l_sun` pass retains canonical `water_t` frequency inputs and each surface's reflection-probe image, runs the shared IW3 FFT/amplitude conversion, and reproduces the native parallax, multi-scale normal, Fresnel water color, reflection, sun-specular, and fog arithmetic. Model draws additionally upload the native 4x4x4-per-entry RGBA8 model-lighting volume layout. First-person DObj batches preserve native `renderFxFlags & 2`, use `r_znear_depthhack`, and draw in the reserved viewmodel depth range. Static entries are per instance; ordinary DObj and live DynEntity model entries are refreshed from their retained/canonical lighting origins. Dynamic brush batches preserve their canonical materials and directional lightmaps; their `GfxDrawSurf` primary-light indices select techniques from the same immutable world light table, and encountered spot-lit moving surfaces reuse the authored primary lightmap, RGB attenuation-atlas row, cone, and normal-map translation. The frontend advances the canonical five-slot campaign fog state and the backend applies the scripted exponential visibility/color blend to world and model geometry. Canonical archived/latched `r_aaSamples` now selects 1×/2×/3×/4× scene multisampling; the browser cap is 4×, unsupported counts fall downward like native `R_SetupAntiAliasing`, and matching RGBA8/depth24 renderbuffers resolve through `GL_NEAREST` into the single-sample scene color/depth targets before post effects and 2D. Browser-owned canvas antialiasing is disabled so 1× is genuinely off. The multisample targets resize and recover with the WebGL2 context. The resolved RGBA8 scene consumes the exact `R_UpdateColorManipulation` constants, canonical `GfxGlow` setup/apply constants, canonical `GfxDepthOfField` ranges, and canonical `refdef.blurRadius` before 2D; glow thresholds/desaturates at quarter resolution and runs a separable Gaussian, while DOF reconstructs view distance from the sampled infinite-projection depth target and applies independent viewmodel, near, and far CoC radii. The second target then composites the sharp sun flare/HUD. `R_SetColorMappings` remains capability-correct: browser registration reports `deviceSupportsGamma=false`, so `r_gamma` is retained and tested but does not incorrectly darken the composited canvas. Canonical cull, depth, color-write, alpha-test/blend, addressing, and filtering states remain per batch. GPU handles stay backend-only; 2D/cubemap textures, water fields, reflection probes, 3D lighting volumes, both shadow partitions, multisample/shadow/post-effect/glow targets, geometry, and instance buffers are recreated after context loss. |
...
| Input | `MODIFIED KISAK` / movement, look, fire, reload, melee, and weapon-cycle gameplay proved plus `WEB PLATFORM IMPLEMENTATION` | Product and diagnostics share one production-safe DOM controller for canonical keyboard, punctuation, function/navigation/modifier/numpad keys, five mouse buttons, wheel pulses, pointer-lock deltas, absolute coordinates, focus release, and disposal. Lossless transitions use a validated one-way Worker message; relative movement is summed once per browser frame without request IDs, timers, or acknowledgements. A failed transport clears local held state and marks the runtime unavailable. Wasm drains input through canonical `CL_KeyEvent` and `CL_MouseEvent`; native bindings, `CG_DrawActiveFrame -> CL_Input`, usercmd creation, `SV_Frame` consumption, prediction, `Pmove`, and weapon state remain authoritative. Fresh browser profiles receive standard movement, stance, lean, combat, grenade, night-vision, objective, and weapon-cycle defaults only when each key is unbound. The browser pump refreshes native `com_frameTime` and accumulates real Worker elapsed time before advancing gameplay at no more than 125 Hz, so faster-than-vsync callbacks neither manufacture server milliseconds nor distort snapped movement. SP `ClientThink_real` rounds `g_gravity` without reinterpreting a platform-dependent `long double`, preserving the canonical 800-unit player gravity on Wasm. `PM_GroundTrace` retains its canonical primary probe; its bounded sub-unit Killhouse support fallback settles the hull onto the contact before publishing grounded state, preventing hovering and slope-projected upward flight. Chrome proves grounded spawn/incline traversal, gravity-driven falls and stable landings, zero velocity and position drift after release, camera rotation, player-origin movement, weapon switching, ammo consumption, reload clip-out/clip-in, and canonical attack/melee bindings crossing the browser boundary with `keyCatchers=0`. Cursor visibility is marshalled back to the DOM host instead of accessing `OffscreenCanvas.style`; absolute/relative mode is published separately from the canonical key catchers because the UI may hide the system arrow while drawing its own cursor. A browser-consumed Escape pointer-lock exit is forwarded exactly once, menu mode releases capture, and absolute motion continues through canonical menu input without recapturing on click. Gamepad collection remains future work. |
...
| Playwright browser tests | `WEB PLATFORM IMPLEMENTATION` | Routine smoke/remainder tiers are disjoint and focus on Worker, storage, lifecycle, bridge, canvas, and WebGL behavior. Product coverage uses real pages for two-tab writer exclusion, clean handoff, terminate-before-release after timeout uncertainty, and stale-generation isolation. DPR 1/1.25/1.5/2, scaled layouts, resize in absolute/relative modes, wheel delta modes, and auxiliary mouse buttons exercise the shipped input path. Startup capability failure is proven to stop before Worker/storage construction. Focused AA coverage verifies genuine 1× disablement, RGBA8/depth24 4× allocation, color resolve, 4× cap, capability fallback, resize, and context recovery without retail assets. `@native-covered` duplicates run only in the explicit exhaustive suite. Synthetic assets only. |
...
and canonical far-distance culling.
...
## Global XModel LOD convergence update (2026-08-24)

All portable XModel submission paths now share the native scene LOD inputs:
`refdef.vieworg`, the `tanHalfFovY * 2.118673086166382` conversion, rigid and
skinned `r_lodScale*`/`r_lodBias*` ramps, placement scale, authored XModel
thresholds, and the `r_forceLod` developer override. DObjs, DynEntity models,
and FX XModels delegate the adjusted distance to `XModelGetLodForDist` and
treat `-1` as distance culling instead of substituting the lowest-detail LOD.

The static-world path no longer uploads only `numLods - 1`. It retains every
authored non-deformed LOD surface once per canonical model and uses the native
static ordering: rigid FOV ramp, `GfxStaticModelDrawInst::cullDist`, then
placement-scale adjustment and `XModelGetLodForDist`. Each frame the WebGL2
backend partitions immutable placements into the selected LOD's instance
range, uploads only when selections change, and reuses the same ranges for the
camera and sun-shadow passes. This is a renderer-platform implementation of
the shared Kisak policy, not a Killhouse weapon-rack exception or a parallel
asset representation. Geometry shared by model/LOD/surface is retained only
once even when reflection-probe groups need separate batches; the bounded
all-LOD command ceiling is one million vertices and three million indices
(about 81 MiB of portable geometry). Focused Wasm tests cover every authored LOD being
retained, near/transition/far selection, rigid versus skinned ramps, placement
scale, static cull distance, invalid inputs, and canonical far culling.
The final Release Chrome Killhouse run retains 238 canonical models across
1,965 all-LOD batches, 603,502 shared vertices, 1,306,836 indices, and 12,188
instances. All 539 encountered images are supported, model lighting covers all
12,188 entries with zero failures, no batch falls back, the gameplay frame
advances after submission, and the weapon wall renders in the live spawn view.
...
`GfxLightDef` identity, falloff-atlas placement, shadow eligibility, and each
`GfxDrawSurf` primary-light identity at the renderer boundary. Per-scene
submission refreshes animated `refdef` light color, origin, direction, radius,
cone, exponent, and shadow eligibility rather than freezing the world copy.
Native `R_SetupMaterial`
...
The bounded external IWI decoder also accepts COD4 format 4 (`L8`), preserves
the native smallest-to-largest mip order, and expands each texel to opaque
`L,L,L`. The canonical renderer copies each LightDef's authored falloff into
the secondary lightmap atlas; the WebGL pass samples that embedded curve
through native `width/512` and `lmapLookupStart/512` placement instead of
binding the standalone image as an extra browser sampler. Chrome
...
through the world tangent basis. The exact authored L8 falloff and canonical
...
lights remain unencountered.

Eligible primary spots now follow COD4's global shadow selection rather than
a Killhouse-specific exception. The backend ranks used spots with
`R_ShadowedSpotLightScore`, respects `sm_spotEnable`/`sm_maxLights`, retains up
to four 512-square depth targets, and builds each projection with the native
`MatrixForViewer` light-axis convention plus the explicit D3D `[0,1]` to
WebGL `[-1,1]` depth conversion. Opaque BSP, static-XModel, ordinary DObj,
DynEntity, and moving-brush casters feed the selected maps; only receivers with
the matching primary-light index apply four-comparison PCF. The authored
primary-lightmap coordinates remain unchanged, matching the native vertex
program rather than introducing an atlas-orientation workaround.
...
## Canonical reflection-probe mip update (2026-08-24)

The environment shader already reproduced the native explicit probe selection
`lod = -8 * specularAlpha + 6`, but the platform image boundary retained only
the largest cubemap face and the WebGL object used a non-mipmapped minification
filter. Consequently every world and XModel material sampled the sharpest
reflection level regardless of its authored roughness. In Killhouse this made
the concrete floor read as broadly wet and raised the apparent scene
brightness.

Both external IWI cubemaps (mip-major, smallest to largest) and canonical DB
load definitions (face-major, largest to smallest) now decode every authored
level atomically within the expanded complete-chain bound. WebGL uploads those
levels in D3D mip order and selects a mipmapped minification filter; generated
mips remain only as a bounded fallback for genuinely single-level cubes.
Focused native fixtures cover both storage orders, level dimensions/colors,
no-mipmap inputs, malformed layouts, and atomic failure. A live Release
Killhouse capture confirms the dark concrete floor is matte again while
retaining localized authored reflections, with no WebGL errors.
...
## Canonical far sun-shadow partition update (2026-08-23)

The WebGL boundary now retains both native-scale sun-shadow partitions. The
existing 1024-square near map keeps `sm_sunSampleSizeNear=0.25`; the second
1024-square map uses IW3's default four-times partition ratio, or 1.0 world
unit per texel. Both projections share the canonical sun axes and world depth
extent, snap independently to their texel grids, and are regenerated from the
same canonical alpha-tested world, static-XModel, ordinary-DObj, DynEntity,
and moving-brush caster commands.
...
## Canonical model primary-light identity update (2026-08-24)

Static-XModel batching now preserves the native grouping key of model,
reflection probe, and primary-light index. The browser command previously
merged instances which differed only by primary light, even though
`R_AddAllStaticModelSurfaces` deliberately splits that case. Ordinary DObj
draws likewise retain the primary-light index returned by the canonical light
grid sample.

A follow-up parity audit also found that the portable static-model builder was
still selecting ordinary `TECHNIQUE_LIT` (slot 7). Native static-model camera
surfaces use surf types 3/4 and therefore select
`TECHNIQUE_LIT_INSTANCED` or its sun/sun-shadow variants (slots 14--16).
The portable selector now follows that routing: unlit-primary instances prefer
slot 14, directional-primary instances prefer slot 16 then slot 15, and only
fall back to the older family when an authored set lacks the instanced row.
This preserves the encountered `lp_i_*`/`lprobe_i_amb_sun_*` shader identity,
state entry, and ambient-probe classification instead of describing static
foliage and vehicles with a non-instanced pass.

The WebGL `lp_*` translation now consumes the alpha channel already stored in
each native 4x4x4 model-lighting entry. For directional-primary batches it
adds `sunDiffuse * primaryVisibility * saturated N dot sun` to the sampled RGB
lighting before the existing native x2 model modulation. This restores the
authored direct-sun contribution on foliage, vehicles, characters, and props
without applying a browser exposure correction or a Killhouse-specific light.
Focused native coverage verifies that primary-light identity splits otherwise
identical static-instance groups and survives into the retained draw command.

## Canonical ambient-probe foliage and detail-map update (2026-08-24)

A bounded diagnostic of the supplied Killhouse material records showed that
the dense grass, bush, pine, and oak population was already present in the
canonical static-XModel command. Its dominant `lprobe_amb_t0c0` pass was being
translated as an ordinary directional `lp_*` pass, which incorrectly applied
`N dot L` to billboard foliage and reduced much of it to nearly black shapes.
The recovered native pixel program doubles the sampled 3D probe and then adds
`probeAlpha * sunDiffuse` without a normal term. The portable descriptor now
retains that shader-family identity and applies the distinct rule to static
and dynamic model batches; other `lp_*` families keep their directional term.
The WebGL translation preserves that operation order rather than adding sun
to the probe before the x2 modulation, which had doubled direct sun and turned
mixed foliage into pale highlights separated by nearly black clusters. A
portable oracle now fixes the ambient-probe arithmetic independently of the
shader string.

The same inspection recovered the omitted `d0` layer used by world and model
materials. Texture routing now prefers the canonical `colorMap` name hash and
retains the named `detailMap`, its sampler, and authored `detailScale` through
the platform boundary. WebGL samples the scaled detail coordinates and uses
the native `color + detail - 0.5` pre-lighting composition. The temporary
shader-word diagnostics are excluded from the build and no retail program or
asset bytes are repository data.

Canonical two-dimensional load definitions now retain their complete authored
mip chain when alpha-tested material recovery needs it. The backend also
preserves one-level `IMG_FLAG_NOMIPMAPS` images as one-level textures instead
of manufacturing transparent-black-averaged levels, and decodes native
nearest-mip versus linear-mip selection plus 2x/4x anisotropy separately. This
keeps foliage coverage and colour stable without a map-specific texture or a
global LOD/exposure override. When the canonical multisample target and
`r_aaAlpha` are active, opaque alpha-tested batches also enable WebGL2
alpha-to-coverage, matching IW3's transparency-AA state boundary. The launcher
no longer limits desktop rendering
to a 1120-pixel-wide debug viewport; it uses the available width up to the
1920-pixel retail comparison resolution, so geometry and texture LOD decisions
are evaluated at a comparable framebuffer size.

A second bounded material/alpha probe isolated the remaining black foliage to
the canonical `mc/mtl_tree_shadow_caster` surface rather than to the canopy
texture: every accepted texel in that dedicated surface is intentionally
black. Native `R_AddXModelSurfacesCamera` excludes XModel materials whose
`cameraRegion` is 3 while retaining them for build-shadowmap submission. The
portable draw descriptor now carries that canonical region. Static and
ordinary-DObj camera passes omit region 3, but the batches and geometry stay
resident for the existing sun/spot shadow depth passes. This is a global
XModel camera rule, not a foliage-material name exception.

## Canonical persistent receiver-mark generation update (2026-08-23)

The browser camera scene now completes native EffectsCore mark generation for
all receiver families reached by the existing fragment seam. World-brush marks
remain generated by `R_UpdateRemainingEffects`; after the canonical physics
stage, `R_RenderScene` additionally invokes the shared static-XModel and
ordinary entity DObj/moving-brush generators before appending the retained mark
mesh. Static receivers use an all-visible byte span consistent with the current
conservative portable static command, while first-person depth-hacked DObjs and
out-of-range entity identities remain excluded. EffectsCore continues to own
mark allocation, lifetime, clipping, material choice, and vertex expansion.

Activating these previously dead Release paths exposed upstream mark-lock
increments and decrements embedded inside `iassert`. Release compilation
removed those expressions and therefore their side effects. The shared mark
code now performs each atomic operation explicitly and checks the returned
count separately, preserving the intended single-owner invariant in every
configuration. The browser boundary also supplies the two narrow canonical
DObj helpers required by the shared generator: pose/hidden-part collection and
`DObjSkelMat` to 4x3 matrix conversion.

Focused Wasm-native mark-mesh and mark-fragment tests pass after the Release
build. The existing Chrome tab boots Killhouse, advances through the armory,
equips the canonical G36C, and fires six gameplay shots into the weapon-rack
receiver. Authored muzzle smoke/glow/flash, dirt/spark impact effects, weapon
audio, and persistent mark generation run without assertion, shader, context,
or draw failures. Static/DObj receiver marks currently use the material's
...
attachment remains sampleable by the canonical depth-of-field pass described
below.
...
## Canonical glow/bloom convergence update (2026-08-23)

The renderer frontend now mirrors `R_SetGlowInfo` and `R_UsingGlow`. It chooses
the active `GfxGlow` from the campaign refdef or `r_glowUseTweaks`, then applies
the native allow/script-force, fullbright, `r_glow`, enable, intensity, and
radius gates. A native-tested portable helper emits the exact
`CONST_SRC_CODE_GLOW_SETUP` cutoff/rescale/desaturation values and
`CONST_SRC_CODE_GLOW_APPLY` intensity rather than asking the WebGL backend to
reinterpret vision-set state.

At the permanent GPU boundary, `glow_consistent_setup` is represented by a
symmetric quarter-resolution reduction that thresholds and desaturates the
resolved 3D scene. Two backend-owned RGBA8 targets then ping-pong through a
17-tap separable Gaussian derived from the authored 480-line radius. The
filtered image is additively applied at the canonical scene-composite point,
after campaign film manipulation and before the sharp sun flare/HUD. Both
targets and the glow program participate in context loss and resize; retained
glow scene parameters reset at map unload. No glow image or WebGL handle
crosses into cgame.

The active Killhouse vision set deliberately reports glow disabled while still
authoring cutoff `0.812775`, intensity `0.5`, and radius `9.2`, so ordinary
play remains unchanged. In the current Chrome tab the Release build renders
all 8,475 surfaces with no shader, framebuffer, context, descriptor, or draw
failure. Temporarily selecting the native tweak path with those same values
activates the pass and visibly produces bounded halos around the armory's
bright ceiling fixtures and weapon windows while the HUD remains sharp; the
tweak override is disabled again after validation.

## Canonical depth-of-field convergence update (2026-08-23)

The renderer frontend now mirrors `R_SetDepthOfField`. It selects the authored
`refdef.dof` ranges only while `r_dof_enable` is active, or the native tweak
ranges while `r_dof_tweak` is active, and preserves the native span and blur
gates before submitting a portable scene descriptor. A focused native/Wasm
oracle validates the independent viewmodel, near, and far circle-of-confusion
radii at their exact boundaries and rejects the native-invalid near blur range.

The first resolved scene composite samples the backend-owned depth attachment
and reconstructs view distance from IW3's infinite projection. Depth-hacked
viewmodel values are decoded from the reserved 1/64 range with
`r_znear_depthhack`; ordinary world depth uses the scene near plane. The
resulting viewmodel, near, and far radii drive the bounded screen-space filter
before film manipulation, glow, sun flare, and sharp HUD composition. No depth
texture or WebGL handle crosses into cgame, and the depth binding participates
in the existing resize/context-loss lifecycle.

Killhouse's normal vision state is correctly inactive because its authored
near range spans exactly one unit. In the current Chrome tab, temporarily
selecting the native tweak defaults activates all three depth ranges and
...
## Fractional shadow-sampler reconstruction checkpoint (2026-08-25)

The WebGL receiver now reproduces D3D9's linear hardware shadow sampler for
both sun and spot maps. The retail material programs issue four shadow taps;
each tap linearly interpolates four binary depth comparisons at its fractional
texel position. The former WebGL translation instead compared one nearest raw
depth texel per tap. That mismatch left the shadow outlines of Killhouse's
indoor spotlights visibly stair-stepped, while earlier sun-only experiments
could not affect the building-specific defect.

The WebGL2 depth targets now use linear comparison sampling at the recovered
half-texel tap locations, with the native 1024-square sun and 512-square spot
dimensions. The three comparison samplers also own dedicated texture units;
bloom, post-processing, flare, and HUD passes cannot replace them with color
textures before returning to the world shader. Depth extents, caster bias,
primary-light visibility rules, and partition coverage remain unchanged. A
focused Wasm oracle covers diagonal and corner comparison patterns, including
clamped fractional coordinates. The Release build, runtime-prefix check,
shader compile, and live Killhouse close-range check pass with both sun and
spot shadows enabled, no WebGL errors, and continuously filtered edges.

## Canonical spot-shadow caster membership checkpoint (2026-08-25)

The fractional comparison change above corrected filtering but did not remove
Killhouse's hard triangular indoor lighting artifact. The remaining mismatch
was shadow-map submission: the browser rendered every opaque BSP batch, every
static XModel, and every dynamic model into every selected spot light. Native
`R_GenerateSortedPrimarySpotShadowDrawSurfs` instead consumes the light-owned
`GfxWorld::shadowGeom[primaryLightIndex]` surface and static-model lists, then
admits dynamic entities only through per-light visibility.

The world command now carries exact uploaded index ranges for each authored
`sortedSurfIndex`, the matching `TECHNIQUE_BUILD_SHADOWMAP_DEPTH` state, and
each authored `smodelIndex`. The WebGL spot pass draws only those BSP ranges
and only retained static instances whose canonical index belongs to the
selected light. Static caster cull and alpha-test state comes from the build-
shadowmap technique rather than the visible receiver pass. Dynamic model
casters are excluded from primary spot maps until their canonical per-light
entity visibility is carried across the boundary; this matches native's
admission rule instead of restoring the previous all-entities heuristic.

The retained retail `lm_spot_sm` receiver equation remains unchanged: an
active dynamic comparison replaces the authored primary-lightmap visibility,
while an unshadowed light uses the authored value. In the browser-only
Killhouse reproduction, the former bright triangle disappears with spot
shadows enabled and authored table/model shadows remain. The Release build and
runtime-prefix check pass with no WebGL, shader, framebuffer, descriptor, or
draw error. Focused Wasm tests cover BSP range translation, shadow technique
state, and canonical static-model membership.
