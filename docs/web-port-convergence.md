...
GfxImage, water, plus LocalizeEntry
...
The production web target contains 67 C/C++ translation units: 35 outside
...
| Database initialization | `MODIFIED KISAK` / partial | The web target compiles real `db_registry.cpp` ownership for `DB_BuildOSPath`, `DB_TryLoadXFileInternal`, `DB_TryLoadXFile`, `DB_Thread`, `DB_LoadXZone`, `DB_LoadZone_f`, `DB_InitThread`, and `DB_LoadXAssets`. It then executes through XFile inflate, PMem blocks, streams, generated list/string/asset dispatch, RawFile, PhysPreset, MaterialTechniqueSet, Material, GfxImage, water, and LocalizeEntry. Narrow synchronous file and Sys context adapters own the platform boundary. Retail `code_post_gfx.ff` publishes assets through index 1199 and reaches asset 1200 type 8 before the uncompiled `Load_SndCurvePtr` family. |
...
| Asset registry | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | Generated RawFile, PhysPreset, MaterialTechniqueSet, Material, GfxImage, and LocalizeEntry paths consume the canonical 32-bit asset-entry pool, per-type pools, free chain, normalized hash table, and zone ownership through `DB_AddXAsset`/`DB_LinkXAssetEntry`. Owned retail publishes 1,201 records through entry 1,216, including LocalizeEntry pool slots 0-1,115, with free entries 32,752 -> 31,551. Gate 2 remains a frozen oracle and is not called by the generated path. |
...
| Portable parser tests | `MODIFIED KISAK` / partial | Empty-list, string-list, RawFile, PhysPreset, TechniqueSet, Material, image, water, and LocalizeEntry fixtures cover pointer forms, dependency ordering, tables, shader/image payloads, direct/interior XStrings, malformed streams/tokens/lengths, pool/entry exhaustion, and failure-before-publication. The same direct executable runs under Win32 x86 and Emscripten; browser tests cover the Worker filesystem boundary. The census dispatcher fuzz target remains independent regression evidence. |
...
eight-byte body/XString closure, failure atomicity, 1,116 retail publications,
and the new SoundCurve boundary. The current
...
MaterialTechniqueSet, Material, GfxImage, water, and LocalizeEntry
publication now use shared Kisak code. The owned retail startup zone naturally
publishes through asset 1199 and stops at asset 1200, type 8
`ASSET_TYPE_SOUND_CURVE`; `Load_SndCurvePtr` is the recommended next
...
| --- | --- | --- |
| Shared or narrowly modified Kisak code in the web target | Increase | Improving: 35 of 67 production translation units are outside `src/web`, now including real `db_registry.cpp`, the DB file-platform seam, generated RawFile/PhysPreset/TechniqueSet/Material/Image/water/LocalizeEntry loading, and canonical registry publication. |
...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High: qcommon plus canonical DB framing, incremental inflate, PMem blocks, stream globals, generated list/string/asset dispatch, RawFile, PhysPreset, TechniqueSet, Material, GfxImage, water, LocalizeEntry, and their real DB publications now execute. Remaining generated families, client, cgame, game, xanim runtime, collision, and script VM remain outside the web target. |
| Native-vs-web semantic comparisons | Increase | The Gate 3 startup closure matches under Win32 x86 and Wasm through RawFile, PhysPreset, TechniqueSet, Material, nested/top-level images, water, and LocalizeEntry. Fixtures cover `-1`, `-2`, insertion, aliases, direct/inline/interior XStrings, dependency ordering, constants/state bits, final-only publication, failure atomicity, and deterministic pool/free-chain deltas. |
