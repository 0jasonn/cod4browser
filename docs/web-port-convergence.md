...
| Playwright browser tests | `WEB PLATFORM IMPLEMENTATION` | Routine smoke/remainder tiers are disjoint and focus on Worker, storage, lifecycle, bridge, canvas, and WebGL behavior. Focused AA coverage verifies genuine 1× disablement, RGBA8/depth24 4× allocation, color resolve, 4× cap, capability fallback, resize, and context recovery without retail assets. `@native-covered` duplicates run only in the explicit exhaustive suite. Synthetic assets only. |
