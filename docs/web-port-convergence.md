...
lifecycle and generated RawFile, PhysPreset, plus MaterialTechniqueSet
...
The production web target contains 62 C/C++ translation units: 30 outside
...
| Database initialization | `MODIFIED KISAK` / partial | The web target compiles real `db_registry.cpp` ownership for `DB_BuildOSPath`, `DB_TryLoadXFileInternal`, `DB_TryLoadXFile`, `DB_Thread`, `DB_LoadXZone`, `DB_LoadZone_f`, `DB_InitThread`, and `DB_LoadXAssets`. It then executes through XFile inflate, PMem blocks, streams, generated list/string/asset dispatch, RawFile, PhysPreset, and MaterialTechniqueSet. Narrow synchronous file and Sys context adapters own the platform boundary. Retail `code_post_gfx.ff` publishes TechniqueSets at assets 0-1 and reaches asset 2 type 4 before the uncompiled `Load_MaterialHandle` family. |
| Canonical database asset ABI | `SHARED KISAK` / partial | `RawFile`, `XAssetHeader`, `XAssetType`, and `XAsset` remain renderer-free. The canonical 44-byte `PhysPreset` is shared from `physics/phys_preset.h`. Canonical `MaterialTechniqueSet`, flexible technique/pass records, vertex declarations, shader load definitions, and material arguments are now shared from `gfx_d3d/material_types.h` without requiring D3D headers. Native x86/Wasm tests cover their exact 32-bit sizes and offsets. Canonical `XAnimParts`, `WeaponDef`, `LocalizeEntry`, XModel, Material, FX, collision, world, image, and light structures remain shared lightweight ABI declarations. |
...
| Asset registry | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | Generated RawFile, PhysPreset, and MaterialTechniqueSet paths consume the canonical 32-bit asset-entry pool, per-type pools, free chain, normalized hash table, and zone ownership through `DB_AddXAsset`/`DB_LinkXAssetEntry`. Owned retail TechniqueSets use pool slots 0-1 and entries 16-17 with free entries 32,752 -> 32,750. Gate 2 remains a frozen oracle and is not called by the generated path. |
...
| `Material` and techniques | `MODIFIED KISAK` / partial | The canonical generated path now owns `MaterialTechniqueSet`, its 34 technique pointers, flexible pass records, vertex declarations, vertex/pixel shader records and bytecode, arguments, pointer aliases, insertion cells, and final DB publication. The web renderer-disabled boundary implements the three canonical post-load hook signatures without D3D objects. Owned retail publishes `sm2/2d` and `2d`, then stops at asset 2 before `Load_MaterialHandle`. Gate 2's broader Material records remain oracle-only. |
...
| Portable parser tests | `MODIFIED KISAK` / partial | Empty-list, string-list, RawFile, PhysPreset, and full MaterialTechniqueSet fixtures cover pointer forms, child ordering, shaders, arguments, malformed stream/pointer/count/type input, pool exhaustion, entry exhaustion, and failure-before-publication. The same direct executable runs under Win32 x86 and Emscripten; browser tests cover the Worker filesystem boundary. The census dispatcher fuzz target remains independent regression evidence. |
...
XFile block table, zone streams, generated prefix, RawFile, PhysPreset, and
MaterialTechniqueSet
publication now use shared Kisak code. The owned retail startup zone naturally
publishes assets 0-1 as type 5 `ASSET_TYPE_TECHNIQUE_SET` and stops at asset 2,
type 4 `ASSET_TYPE_MATERIAL`; `Load_MaterialHandle` is the recommended next
generated-family slice after architecture review.
...
| --- | --- | --- |
| Shared or narrowly modified Kisak code in the web target | Increase | Improving: 30 of 62 production translation units are outside `src/web`, now including real `db_registry.cpp`, the DB file-platform seam, generated RawFile/PhysPreset/MaterialTechniqueSet loading, and canonical registry publication. |
...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High: qcommon plus canonical DB framing, incremental inflate, PMem blocks, stream globals, generated list/string/asset dispatch, RawFile, PhysPreset, MaterialTechniqueSet, and their real DB publications now execute. Remaining generated families, client, cgame, game, xanim runtime, collision, and script VM remain outside the web target. |
| Native-vs-web semantic comparisons | Increase | The Gate 3 startup closure still matches under Win32 x86 and Wasm. Both platforms consume the same RawFile, PhysPreset, and MaterialTechniqueSet fixtures and emit the same result through `-1`, `-2`, insertion, top-level and child aliases, direct/inline XStrings, renderer child ordering, final publication, failure atomicity, and deterministic pool/free-chain deltas. |
