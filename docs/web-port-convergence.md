...
The production web target contains 66 C/C++ translation units: 34 outside
...
| Database initialization | `MODIFIED KISAK` / partial | The web target compiles real `db_registry.cpp` ownership for `DB_BuildOSPath`, `DB_TryLoadXFileInternal`, `DB_TryLoadXFile`, `DB_Thread`, `DB_LoadXZone`, `DB_LoadZone_f`, `DB_InitThread`, and `DB_LoadXAssets`. It then executes through XFile inflate, PMem blocks, streams, generated list/string/asset dispatch, RawFile, PhysPreset, MaterialTechniqueSet, Material, GfxImage, and water. Narrow synchronous file and Sys context adapters own the platform boundary. Retail `code_post_gfx.ff` publishes through asset 4 and reaches asset 5 type 22 before the uncompiled `Load_LocalizeEntryPtr` family. |
...
| Asset registry | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | Generated RawFile, PhysPreset, MaterialTechniqueSet, Material, and GfxImage paths consume the canonical 32-bit asset-entry pool, per-type pools, free chain, normalized hash table, and zone ownership through `DB_AddXAsset`/`DB_LinkXAssetEntry`. Owned retail uses entries 16-21 and TechniqueSet/Material/Image pool slots 0-1/0/0-2, with free entries 32,752 -> 32,746. Gate 2 remains a frozen oracle and is not called by the generated path. |
...
| Portable parser tests | `MODIFIED KISAK` / partial | Empty-list, string-list, RawFile, PhysPreset, TechniqueSet, Material, image, and water fixtures cover pointer forms, dependency ordering, tables, shader/image payloads, malformed streams/tokens/counts, pool/entry exhaustion, and failure-before-publication. The same direct executable runs under Win32 x86 and Emscripten; browser tests cover the Worker filesystem boundary. The census dispatcher fuzz target remains independent regression evidence. |
...
MaterialTechniqueSet, Material, GfxImage, and water
publication now use shared Kisak code. The owned retail startup zone naturally
publishes assets 0-4 and stops at asset 5, type 22
`ASSET_TYPE_LOCALIZE_ENTRY`; `Load_LocalizeEntryPtr` is the recommended next
...
| --- | --- | --- |
| Shared or narrowly modified Kisak code in the web target | Increase | Improving: 34 of 66 production translation units are outside `src/web`, now including real `db_registry.cpp`, the DB file-platform seam, generated RawFile/PhysPreset/TechniqueSet/Material/Image/water loading, and canonical registry publication. |
...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High: qcommon plus canonical DB framing, incremental inflate, PMem blocks, stream globals, generated list/string/asset dispatch, RawFile, PhysPreset, TechniqueSet, Material, GfxImage, water, and their real DB publications now execute. Remaining generated families, client, cgame, game, xanim runtime, collision, and script VM remain outside the web target. |
| Native-vs-web semantic comparisons | Increase | The Gate 3 startup closure matches under Win32 x86 and Wasm through RawFile, PhysPreset, TechniqueSet, Material, nested/top-level images, and water. Fixtures cover `-1`, `-2`, insertion, aliases, direct/inline XStrings, dependency ordering, constants/state bits, final-only publication, failure atomicity, and deterministic pool/free-chain deltas. |
