...
| Audio | `MODIFIED KISAK` / partial plus `WEB PLATFORM IMPLEMENTATION` | Canonical `SND_InitDriver -> renderer registration -> SND_Init` ordering now runs in Wasm and parses the real DB-owned `soundaliases/channels.def`. Emscripten OpenAL owns the browser audio context; autoplay unlock/resume remains a platform concern. |
