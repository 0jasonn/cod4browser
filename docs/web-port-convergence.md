...
| Asset registry | `MODIFIED KISAK` / partial plus `TEMPORARY WEB SUBSTITUTE` oracle | Generated RawFile, PhysPreset, MaterialTechniqueSet, Material, GfxImage, LocalizeEntry, SndCurve, sound alias, LoadedSound, and Font paths consume the canonical 32-bit asset-entry pool, per-type pools, free chain, normalized hash table, and zone ownership through `DB_AddXAsset`/`DB_LinkXAssetEntry`. Owned retail publishes 1,240 records through entry 1,255, with free entries 32,752 -> 31,512. Gate 2 remains a frozen oracle and is not called by the generated path. |
...
four canonical families and the new FX/XModel architectural boundary. The
current
...
sound alias, LoadedSound, and Font publication now use shared Kisak code. The
owned retail startup zone publishes 1,240 assets and stops at asset 1224,
type 25 `ASSET_TYPE_FX`; `Load_FxEffectDefHandle` reaches the still-uncompiled
XModel graph and is the current major architectural decision boundary.
...
| --- | --- | --- |
| Shared or narrowly modified Kisak code in the web target | Increase | Improving: 40 of 72 production translation units are outside `src/web`, now including real `db_registry.cpp`, the DB file-platform seam, generated RawFile/PhysPreset/TechniqueSet/Material/Image/water/LocalizeEntry/Sound/Font loading, and canonical registry publication. |
...
| Native engine systems not compiled | Decrease sharply after the GfxWorld proof | High: qcommon plus canonical DB framing, incremental inflate, PMem blocks, stream globals, generated list/string/asset dispatch, RawFile, PhysPreset, TechniqueSet, Material, GfxImage, water, LocalizeEntry, SoundCurve, sound aliases, LoadedSound, Font, and their real DB publications now execute. Remaining generated families, client, cgame, game, xanim runtime, collision, and script VM remain outside the web target. |
| Native-vs-web semantic comparisons | Increase | The Gate 3 startup closure matches under Win32 x86 and Wasm through RawFile, PhysPreset, TechniqueSet, Material, nested/top-level images, water, LocalizeEntry, SoundCurve, sound aliases, LoadedSound, and Font. Fixtures cover `-1`, `-2`, insertion, aliases, direct/inline/interior XStrings, dependency ordering, payloads, final-only publication, failure atomicity, and deterministic pool/free-chain deltas. |
