...
| Audio | `MODIFIED KISAK` / gameplay LoadedSound bridge plus `WEB PLATFORM IMPLEMENTATION` | Canonical `SND_InitDriver -> renderer registration -> SND_Init` ordering runs in the Worker and keeps alias selection, channel state, attenuation, playback IDs, and zone-owned LoadedSound PCM in Kisak. The KISAK_WEB OpenAL-compatible proxy mirrors only driver-visible source/buffer state and sends bounded typed PCM commands to main-thread `WebAudioDriver`, which owns AudioContext/AudioBuffer/AudioNode resources and gesture resume. Release Chrome proves loaded 16-bit mono/stereo weapon fire, material-specific impacts, reload clip-out/clip-in, ambient, and UI playback from canonical aliases. Streaming, reverb, and full OpenAL parity remain unsupported. |
