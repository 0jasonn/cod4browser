...
int com_safemode = 0;
int com_fullyInitialized = 0;
int com_lastFrameTime[4]{};
const dvar_t *loc_language = nullptr;
extern bool isLoadingAutoExecGlobalFlag;
extern char *com_consoleLines[32];
extern int32_t dvar_modifiedFlags;
static std::string g_runtimeTrace;
static int g_writeFile = 7;
static bool g_hasProfile = false;
static std::array<sysEvent_t, 4> g_events{};
static std::size_t g_eventIndex = 0;
static char *g_eventPayload = nullptr;

void __cdecl Com_Quit_f() {}
int __cdecl FS_FOpenFileWrite(const char *filename)
{
    g_runtimeTrace += std::string("open:") + filename + "\n";
    return g_writeFile;
}
int __cdecl FS_FOpenFileWriteToDir(const char *filename, const char *directory)
{
    return FS_FOpenFileWrite((std::string(directory) + "/" + filename).c_str());
}
void FS_Printf(int file, const char *format, ...)
{
    assert(file == 7);
    char text[256];
    va_list args;
...
void __cdecl FS_FCloseFile(int file) { FS_Printf(file, "close\n"); }
void __cdecl Key_WriteBindings(int32_t client, int32_t file) { FS_Printf(file, "bindings:%d\n", client); }
void __cdecl Con_WriteFilterConfigString(int32_t file) { FS_Printf(file, "filters\n"); }
void __cdecl Dvar_WriteVariables(int file) { FS_Printf(file, "variables\n"); }
void __cdecl Dvar_WriteDefaults(int file) { FS_Printf(file, "defaults\n"); }
bool __cdecl Com_HasPlayerProfile() { return g_hasProfile; }
int Com_BuildPlayerProfilePath(char *path, int size, const char *format, ...)
{
    assert(std::strcmp(format, "config.cfg") == 0);
    return std::snprintf(path, size, "profiles/test/%s", format);
}
int32_t __cdecl CL_ControllerIndexFromClientNum(int32_t client) { assert(client == 0); return 0; }
void __cdecl Scr_Settings(int, int, int) {}
sysEvent_t *__cdecl Sys_GetEvent(sysEvent_t *result)
{
    assert(g_eventIndex < g_events.size());
    *result = g_events[g_eventIndex++];
    return result;
}
void __cdecl CL_KeyEvent(int32_t client, int32_t key, int32_t down, uint32_t time)
{
    assert(client == 0 && key == 42 && down == 1 && time == 123);
    g_runtimeTrace += "key\n";
}
void __cdecl CL_CharEvent(int32_t client, int32_t key)
{
    assert(client == 0 && key == 'x');
    g_runtimeTrace += "char\n";
}
void Z_Free(void *ptr, int type)
{
    assert(ptr && ptr == g_eventPayload && type == 10);
    std::memset(ptr, 0, std::strlen(g_eventPayload));
    std::free(ptr);
    g_eventPayload = nullptr;
    g_runtimeTrace += "free\n";
}
void __cdecl ObserveExec()
{
    assert(Cmd_Argc() == 2 && Cmd_LocalClientNum() == 0);
    assert(isLoadingAutoExecGlobalFlag == (std::strcmp(Cmd_Argv(1), "autoexec_dev.cfg") == 0));
    g_runtimeTrace += std::string(Cmd_Argv(1)) + "\n";
}
void __cdecl ObserveConsole() { assert(Cmd_Argc() == 1); g_runtimeTrace += "console\n"; }
#endif
...
int main()
{
    ...
    Cbuf_ExecuteBuffer(0, 0, "writeconfig test");
    const std::string configBody = "// generated by Call of Duty, do not modify\n"
        "unbindall\nbindings:0\nvariables\nfilters\nclose\n";
    assert(g_runtimeTrace == "open:players/test.cfg\n" + configBody);
    g_runtimeTrace.clear();
    Cbuf_ExecuteBuffer(0, 0, "writedefaults defaults.cfg");
    assert(g_runtimeTrace == "open:defaults.cfg\n// generated by Call of Duty, do not modify\n"
        "defaults\nclose\n");
    g_runtimeTrace.clear();
    Cbuf_ExecuteBuffer(0, 0, "writeconfig;writedefaults a b");
    assert(g_runtimeTrace.empty());
    g_writeFile = 0;
    Cbuf_ExecuteBuffer(0, 0, "writeconfig failed;writedefaults failed");
    assert(g_runtimeTrace == "open:players/failed.cfg\nopen:failed.cfg\n");
    g_writeFile = 7;
    g_runtimeTrace.clear();

    dvar_modifiedFlags |= DVAR_ARCHIVE;
    Com_WriteConfiguration(0);
    assert(g_runtimeTrace.empty() && (dvar_modifiedFlags & DVAR_ARCHIVE));
    ...
    dvar_modifiedFlags |= DVAR_ARCHIVE;
    Com_WriteConfiguration(0);
    assert(g_runtimeTrace == "open:players/profiles/test/config.cfg\n" + configBody);
    assert(!(dvar_modifiedFlags & DVAR_ARCHIVE));
    com_fullyInitialized = 0;
    g_runtimeTrace.clear();

    // Observe only the filesystem boundary; tokenization and execution order
    // still pass through the real registered exec command and command buffer.
    cmd_function_s *exec = Cmd_FindCommand("exec");
    const auto originalExec = exec->function;
    exec->function = ObserveExec;
    char safeCommandLine[] = "safe";
    Com_ParseCommandLine(safeCommandLine);
    Com_ExecStartupConfigs(0, "profile.cfg");
    assert(g_runtimeTrace == "default.cfg\nlanguage.cfg\nprofile.cfg\nautoexec_dev.cfg\nsafemode.cfg\n");
    assert(!safeCommandLine[0] && !isLoadingAutoExecGlobalFlag && !Com_SafeMode());
    g_runtimeTrace.clear();
    Com_ExecStartupConfigs(0, nullptr);
    assert(g_runtimeTrace == "default.cfg\nlanguage.cfg\nautoexec_dev.cfg\n");
    char restartCommandLine[] = "unrelated+dVaR_ReStArT";
    Com_ParseCommandLine(restartCommandLine);
    assert(Com_SafeMode() && !*com_consoleLines[1] && !Com_SafeMode());
    com_safemode = 1;
    assert(Com_SafeMode());
    com_safemode = 0;
    exec->function = originalExec;

    g_runtimeTrace.clear();
    static cmd_function_s eventCommand{};
    Cmd_AddCommandInternal("gate3_event", ObserveConsole, &eventCommand);
    g_eventPayload = static_cast<char *>(std::malloc(sizeof("gate3_event")));
    assert(g_eventPayload);
    std::strcpy(g_eventPayload, "gate3_event");
    g_events = {{{123, SE_KEY, 42, 1, 0, nullptr}, {124, SE_CHAR, 'x', 0, 0, nullptr},
        {125, SE_CONSOLE, 0, 0, sizeof("gate3_event"), g_eventPayload},
        {126, SE_NONE, 0, 0, 0, nullptr}}};
    Com_EventLoop();
    assert(g_eventIndex == g_events.size() && !g_eventPayload);
    assert(g_runtimeTrace == "key\nchar\nfree\n");
    Cbuf_Execute(0, 0);
    assert(g_runtimeTrace == "key\nchar\nfree\nconsole\n");
    Cmd_RemoveCommand("gate3_event");
    std::puts("canonical-common config=ordered startup=ordered events=ordered console-payload=released");
#endif
