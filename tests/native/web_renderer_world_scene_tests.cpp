...
void TestAuthoredSoftParticleBindings()
{
    ...
    assert(soft.depthFeather);
    // The non-feathered halo shares the angle/color arithmetic, but has no
    // depth sampler or feather constants and must not request a float-Z pass.
    vs.name="vertcol_simple_foa_dtex.hlsl"; ps.name="vertcol_simple_add.hlsl";
    tech.passArray[0].args=args+3; tech.passArray[0].stableArgCount=6;
    assert(WebRenderer_GetParticleMaterial(&material,5,soft));
    assert(soft.flags==6 && !soft.depthFeather && soft.feather[0]==0);
    assert(soft.falloff[2]==2 && soft.beginColor[0]==1 && soft.endColor[1]==1);
    args[4].dest=99;
    assert(!WebRenderer_GetParticleMaterial(&material,5,soft));
    args[4].dest=13;
    ...
    assert(WebRenderer_GetParticleMaterial(&material,5,soft) && soft.sceneFog);
    ...
    assert(!WebRenderer_GetParticleMaterial(&material,5,soft));
    tech.passCount=1;
    assert(!WebRenderer_GetParticleMaterial(nullptr,5,soft));
    assert(!WebRenderer_GetParticleMaterial(&material,34,soft));
